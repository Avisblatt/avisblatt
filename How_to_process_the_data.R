...
#-------------------------------
# (7) Adding persona and location metadata generated by flair
#-------------------------------

AVIS_YEARS = c(1816, 1839)
start <- Sys.time()
add_flair_data(AVIS_YEARS)
message(sprintf("Took %s minutes", 
                round(difftime(Sys.time(),start, units = "min"),2)))


#-------------------------------
# (8) Extracting main added metadata for transfer to Hasdai
#-------------------------------

# Newly generated information for each ad such as 
...
start <- Sys.time()
create_hasdai_annotations(1729, data_version = 4.2, all_in_one = FALSE)
message(sprintf("Took %s minutes", 
                round(difftime(Sys.time(),start, units = "min"),2)))

start <- Sys.time()
create_hasdai_annotations(1729:1790, data_version = 4.2, all_in_one = TRUE)
create_hasdai_annotations(1791:1820, data_version = 4.2, all_in_one = TRUE)
create_hasdai_annotations(1821:1844, data_version = 4.2, all_in_one = TRUE)
message(sprintf("Took %s minutes", 
                round(difftime(Sys.time(),start, units = "min"),2)))