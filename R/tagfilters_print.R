...
tagfilter_print2 <- function(){
  ...
  dict$neg <- list(
    ...
    englisch(|e|en) gedruckt(|e|en)|französisch(|e|en) gedruckt(|e|en)"
  )
  create_filter_output(dict)
}
...
tagfilter_print3 <- function(){
  ...
  dict$pos <- list(
    ...
    format_3 = "4°|8°|Tom\\.\\b|tom\\.\\b|Tomis|Tomes|Tome|Tomi|(O|o)ctavo|\\bBogen|^(1-9) Bögen|Halbfranzband|^(ein|un)gebunden.|\\bbrosch(.|iert)",
  ...
  dict$neg <- list(
    ...
    person = "Buchhändler|Buchdrucker|Buchb(e|i)nder",
    names = "Joh(\\.|an|ann) Jak(\\.|ob) Flick|Schweighauser.|Bey Hrn\\. Jakob Bernoulli im Engelh(o|oo)f",
    ...
    other_2 = "Oefelin|Rohre|Schuffe|chuffe|Geschir|Meldung",
    ...
    other_9 = "mit Beschläg*. zu einem Bogen|Armbrust-Bogen"
  )
  create_filter_output(dict)
}
